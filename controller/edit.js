    ...
    if (!req.session.username){
        ...
        switch(req.body.type){
            ...
            case "currentPassCheck":{
                let query = "SELECT password FROM users WHERE username = $1 AND password = $2";
...
const insertChanges = (res, fields, req) => {
    let promises = [];
    if (fields.newPass){
        let query = "UPDATE users SET password = $1 WHERE username = $2";
    ...
    if (fields.username){
        let query = "UPDATE users SET username = $1 WHERE username = $2";
...
const deleteAccount = (req, res) => {
    let promises = [], query = "DELETE FROM users WHERE username = $1", 
    params = [req.session.username];
...
const getVisibility = (username, change, response) => {
    let query = "SELECT public FROM users WHERE username = $1";
    let params = [username];
...
const submitState = (username, state, res) => {
    let query, params = [];
    query = "UPDATE users SET public = $1 WHERE username = $2";
