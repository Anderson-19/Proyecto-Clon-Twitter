...
const postTweetText = (req, res) => {
    if (!req.session.username){
        ...
        if (req.headers.option !== "nada"){
            query = "INSERT INTO tweets(username, text, tweet_data, referencedtweet, type) VALUES($1, $2, now(), $3, $4) RETURNING tweetid";
            params = [req.session.username, req.body.text, req.headers.option, req.body.type];
        }
        else{
            query = "INSERT INTO tweets(username, text, tweet_data, type) VALUES($1, $2, now(), $3) RETURNING tweetid";
...
const checkVisibility = (username, res, from, sessionUser, tweets) => {
    let query = "SELECT username, public FROM users WHERE username = ANY($1::VARCHAR[])";
    let params = [username];
...
const doSelect = (username, res) => {
    let query = "SELECT * FROM tweets WHERE username = $1 ORDER BY tweet_data DESC";
    let params = [username];
    db.query(query, params, (err, success) => {
        if (!err){
...
const getFeedTweets = (results, res) => {
    let query = "SELECT * FROM tweets WHERE username = ANY($1::varchar[]) ORDER BY tweet_data DESC";
    let params = [results];
    db.query(query, params, (err, success) => {
        if (!err){
            res.status(200).send(success.rows);
...
const getTweetByText = (req, res) => {
    if (!req.session.username){
        ...
        let query = "SELECT * FROM tweets WHERE text like $1 ORDER BY tweet_data DESC";
        let params = ['%' + req.body + '%'];
        db.query(query, params, (err, success) => {
            if (!err){
                let usernames = [];
    ...
    switch(currentStatus){
        case "Like":{
            if (body.action === "Like"){
                ...
                query = "UPDATE likes SET type = 'Dislike' WHERE referencedtweet = $1 AND username = $2";
            }
            break;
        }
        case "Dislike":{
            if (body.action === "Like"){
                query = "UPDATE likes SET type = 'Like' WHERE referencedtweet = $1 AND username = $2";
            }
            else{
                query = "DELETE FROM likes WHERE referencedTweet = $1 AND username = $2";
            }
...
const InteractionCount = (req, res) => {
    if (!req.session.username){
        ...
        switch(req.body.type){
            case "Like":{
                query = "SELECT * FROM likes WHERE referencedtweet = $1 AND type = 'Like'";
                break;
            }
            case "Dislike":{
                query = "SELECT * FROM likes WHERE referencedtweet = $1 AND type = 'Dislike'";
                break;
            }
            case "Comments":{
