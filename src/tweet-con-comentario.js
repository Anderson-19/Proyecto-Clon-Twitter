...
class ComentarioRetweet extends React.Component{
    constructor(props){
        super(props);
        ...
        return(
            <div>
                <div id={"edition" + this.props.tweetid} hidden={true} style={{position:"absolute"}} >
                    <img id={"delete" + this.props.tweetid} alt={""} onClick={this.borrarTweet} style={{cursor:"pointer", width:"40px", position: "relative", left:"480px"}} src={"https://vectorified.com/images/delete-icon-png-4.png"}/>
                    <img id={"edit" + this.props.tweetid} alt={""} onClick={this.editarTweet} style={{cursor:"pointer", width:"37px", position: "relative", left:"400px"}} src={"https://cdn.pixabay.com/photo/2017/06/06/00/33/edit-icon-2375785_640.png"} />
                </div>
                <div id={"SharedComment"}>
                    ...
                    <div id={"interactions"} style={{textAlign:"justify"}}>
                        ...
                        <img alt={""} onClick={this.comentarioTweet} style={{cursor:"pointer", width:"50px", position:"relative", top:"-25px", left:"220px"}} src={"https://cdn3.iconfinder.com/data/icons/social-messaging-ui-color-shapes-2-1/254000/77-512.png"}/>
                        <img alt={""} onClick={this.compartir} style={{cursor:"pointer", width:"50px", position:"relative", top:"-25px", left:"260px"}} src={"https://pngimg.com/uploads/share/share_PNG31.png"}/>
    ...
    componentDidMount(){
        ...
        this.obtenerComentarioTweet();
        this.ObtenerInteracciones();
    }
    obtenerComentarioTweet = () => {
        ...
        .then(json => {
            this.renderizarDatosComentados(json.username, json.filelocation, json.text, json.tweet_date);
            this.obtenerTweetComentado(json.referencedtweet);
        })
    }
    obtenerTweetComentado = (id) => {
        fetch("http://localhost:5000/makeApost/getTweetById", {
            method:"POST",
            body:id,
            credentials:"include"
        ...
        .then(json => {
            ...
            this.renderizarDatosComentados(json.username, json.filelocation, json.text);
        })
    }
    renderizarDatosComentados = (username, filelocation, text) => {
    ...
    renderizarDatosComentario = (username, filelocation, text, date) => {
    ...
    compartir = () => {
        let id = this.props.type === "rtWithComment2" ? this.props.referencedtweet : this.props.tweetid;
        ReactDOM.render(<EscribirTweet tweetid={id} rtWithComment={true}/>, document.querySelector("#Div-1"));
    }
    comentarioTweet = () => {
        let id = this.props.type === "rtWithComment2" ? this.props.referencedtweet : this.props.tweetid;
        ReactDOM.render(<EscribirTweet tweetid={id} isComment={true}/>, document.querySelector("#Div-1"));
    }
    focusTweet = () => {
        let id = this.props.type === "rtWithComment2" ? this.props.referencedtweet : this.props.tweetid;
        ReactDOM.render(<Enrutar target={"focusedTweet"} referencedTweet={id}/>, document.querySelector("#Div-1"));
    }
    focusOriginal = () => {
    ...
    ObtenerInteracciones = () => {
        ...
        promises.push(this.fetchInteracciones(likesBody));
        promises.push(this.fetchInteracciones(dislikesBody));
        promises.push(this.fetchInteracciones(commentsBody));
        promises.push(this.fetchInteracciones(sharesBody));
    ...
    fetchInteracciones = (body) => {
        let promise = fetch("http://localhost:5000/makeApost/Interactions",{
            method:"POST",
    ...
    enviarInteraccion = (body) =>{
        fetch("http://localhost:5000/makeApost/LikeDislike", {
            method:"POST",
            body:JSON.stringify(body),
            credentials:"include",
            headers:{
            ...
            if (response.status === 200) this.ObtenerInteracciones();
        })
    }
    editarTweet = () => {
        let id = this.props.type === "rtWithComment2" ? this.props.referencedtweet : this.props.tweetid;
        ReactDOM.render(<EscribirTweet edit={true} tweetid={id}/>, document.querySelector("#Div-1"));
    }
    borrarTweet = () => {
        let id = this.props.type === "rtWithComment2" ? this.props.referencedtweet : this.props.tweetid;
        fetch("http://localhost:5000/DataEdition/deleteTweet", {
            method:"POST",
            credentials:"include",
            body:id
        }).then(response => {
            if (response.status === 200){
                ReactDOM.render(<Enrutar target={"feed"}/>, document.querySelector("#Div-1"));
            }
        })
    }
}

export {ComentarioRetweet}